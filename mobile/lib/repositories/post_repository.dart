...
class PostRepository {
  ...
  Future<PostModel> createPost(CreatePostModel post) async {
    try {
      ...
      throw e.toAppException('Failed to create post');
    }
  }

  Future<PaginateDataModel<FeedItemModel>> fetchPosts({
    String? cursor,
    int? limit,
  }) async {
    try {
      ...
      throw e.toAppException('Failed to load posts');
    }
  }
  ...
  Future<({int likesCount})> togglePostLike(PostModel post) async {
    try {
      ...
      throw e.toAppException(
        post.likedByMe == true ? 'Failed to unlike post' : 'Failed to like post',
      );
    }
  }
  ...
  Future<({int repostsCount})> togglePostRepost(
    ...
    try {
      ...
      throw e.toAppException(
        post.repostedByMe == true ? 'Failed to remove repost' : 'Failed to repost',
      );
    }
  }
}
